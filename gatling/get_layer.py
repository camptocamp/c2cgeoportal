...
schemas = [
    "ai", "az", "bb", "bn", "br", "bu", "cb", "cn", "co", "cr", "ct", "ec", "ee", "epa", "ep", "es", "ey",
    "gg", "gn", "gr", "gv", "jp", "lc", "lr", "ly", "md", "nstcm", "ol", "oo", "ou", "pe", "px", "pz", "tr",
    "ts", "tz", "vc", "vy"]
# number instance
...
def main():
    ...
    for _, schema in instance_schema:
        conn = psycopg2.connect("dbname=geoportal_dev")
        cur = conn.cursor()
        cur.execute(
            "select t.name from {schema}.layer as l, {schema}.treeitem as t where l.id = t.id and "
            "l.\"layerType\" = 'internal WMS'".format(schema=schema))
        layers[schema] = []
        for record in cur:
            layers[schema].append(record[0])

        cur.execute(
            "select t.name, l.\"legendRule\" from {schema}.layer as l, {schema}.treeitem as t "
            "where l.id = t.id and l.\"layerType\" = 'internal WMS' and  l.\"legendRule\" IS NOT NULL"
