...
./docker-run pcreate --scaffold=c2cgeoportal_create /tmp/travis/nondockertestgeomapfish \
    --package-name testgeomapfish > /dev/null
./docker-run pcreate --scaffold=c2cgeoportal_nondockercreate /tmp/travis/nondockertestgeomapfish \
    --overwrite --package-name testgeomapfish > /dev/null
./docker-run pcreate --scaffold=c2cgeoportal_update /tmp/travis/nondockertestgeomapfish \
    --package-name testgeomapfish > /dev/null # on create
./docker-run pcreate --scaffold=c2cgeoportal_nondockerupdate /tmp/travis/nondockertestgeomapfish \
