...
APP_PARTIALS_FILES := $(shell find $(PACKAGE)/static-ngeo/js -type f -name '*.html')
LESS_FILES = $(shell find $(PACKAGE)/static-ngeo/less -type f -name '*.less' 2> /dev/null)
JSON_CLIENT_LOCALISATION_FILES = $(addprefix $(APP_OUTPUT_DIR)/locale/, $(addsuffix /$(PACKAGE).json, $(LANGUAGES)))
NGEO_LIBS_JS_FILES += \
...
$(PACKAGE)/locale/$(PACKAGE)-server.pot: $(SERVER_LOCALISATION_SOURCES_FILES) $(VENV_BIN)/pot-create lingua-server.cfg
	$(VENV_BIN)/pot-create -c lingua-server.cfg -o $@ $(SERVER_LOCALISATION_SOURCES_FILES)
...
$(PACKAGE)/locale/$(PACKAGE)-client.pot: \
	...
	$(VENV_BIN)/pot-create -c lingua-client.cfg -o $@ development.ini $(APP_HTML_FILES) $(APP_JS_FILES) $(APP_PARTIALS_FILES)
	# removes the always changed date line
	sed -i '/^"POT-Creation-Date: /d' $@
	sed -i '/^"PO-Revision-Date: /d' $@
...
.PHONY: build-ngeo
build-ngeo: \
	$(APP_OUTPUT_DIR)/build.js \
	$(APP_OUTPUT_DIR)/build.min.css \
	compile-js-catalog
...
$(APP_OUTPUT_DIR)/locale/%/$(PACKAGE).json: $(PACKAGE)/locale/%/LC_MESSAGES/$(PACKAGE)-client.po .build/node_modules.timestamp
	mkdir -p $(dir $@)
	node tools/compile-catalog $< > $@

$(APP_OUTPUT_DIR)/build.js: .build/node_modules.timestamp .build/build.js $(NGEO_LIBS_JS_FILES) $(NGEO_PROJECT_LIBS_JS_FILES)
	mkdir -p $(dir $@)
	awk 'FNR==1{print ""}1' $(NGEO_LIBS_JS_FILES) $(NGEO_PROJECT_LIBS_JS_FILES) .build/build.js > $@
	sed -i '/^\/\/# sourceMappingURL=.*\.map$$/d' $@

$(APP_OUTPUT_DIR)/build.min.css: $(LESS_FILES) .build/node_modules.timestamp
	mkdir -p $(dir $@)
