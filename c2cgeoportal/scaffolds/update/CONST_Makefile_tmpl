...
$(PACKAGE)/locale/$(PACKAGE)-db.pot: .build/venv/bin/db2pot development.ini .build/config.yaml
	mkdir -p $(dir $@)
...
$(PACKAGE)/static/mobile/build/production/App/app.js: $(JSBUILD_MOBILE_OUTPUT_FILES) \
	$(PACKAGE)/static/mobile/config.js $(PACKAGE)/static/mobile/custom.scss $(PACKAGE)/static/mobile/app/view/Settings.js
	rm -rf $(PACKAGE)/static/mobile/build
	rm -rf $(PACKAGE)/static/mobile/archive
...
.build/requirements.timestamp: $(EGGS_DEPENDENTIES)
...
.build/apache.timestamp: $(CONF_FILES) $(PY_FILES) $(APACHE_CONF_DIR)/$(INSTANCE_ID).conf .build/requirements.timestamp
