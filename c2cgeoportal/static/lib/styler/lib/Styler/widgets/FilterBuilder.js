...
Styler.FilterBuilder = Ext.extend(Ext.Panel, {
    ...
    /**
     * Property: additional combo properties.
     */
    comboConfig: {},
    ...
    initComponent: function() {
        ...
        this.items = [this.createChildFiltersPanel()];
    ...
    createEditingMenu: function(layer) {
        ...
        if (this.geometryTypes.indexOf('point')>-1) {
            items.push(new Ext.menu.CheckItem(new GeoExt.Action({
                ...
                map: this.map,
                group: "querier",
                text: OpenLayers.i18n("from_point"),
                iconCls: "point"
            })));
        }
        if (this.geometryTypes.indexOf('line')>-1) {
            items.push(new Ext.menu.CheckItem(new GeoExt.Action({
                ...
                map: this.map,
                group: "querier",
                text: OpenLayers.i18n("from_line"),
                iconCls: "linestring"
            })));
        }
        if (this.geometryTypes.indexOf('polygon')>-1) {
            items.push(new Ext.menu.CheckItem(new GeoExt.Action({
                ...
                map: this.map,
                group: "querier",
                text: OpenLayers.i18n("from_polygon"),
                iconCls: "polygon"
            })));
        }
        if (this.cookieProvider) {
            var item = new Ext.menu.CheckItem({
                text: OpenLayers.i18n("from_stored_geometry"),
    ...
    createBuilderTypeCombo: function() {
        ...
        for(var i=0; i<numTypes; ++i) {
            type = types[i];
            data[i] = [type, OpenLayers.i18n(this.builderTypeNames[type])];
        }
        return Ext.apply({
            xtype: "combo",
    ...
    newRow: function(filterPanel) {
        ...
        var firstItems = [{
            ...
            items: [{
                xtype: "button",
                tooltip: OpenLayers.i18n("remove condition"),
                cls: 'x-btn-icon',
                iconCls: "delete",
                handler: function() {
