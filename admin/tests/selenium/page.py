...
class BasePage(object):
    ...
    def select_language(self, locale):
        ...
        self.find_element(By.XPATH, '//a[contains(@href,"language={}")]'.format(locale)).click()

    def wait_jquery_to_be_active(self):
        WebDriverWait(self.driver, 10).until(
            lambda driver: driver.execute_script("return (window.jQuery != undefined && jQuery.active == 0)")
        )
    ...
    def check_pagination_info(self, expected, timeout):
        WebDriverWait(self.driver, timeout).until(
            expected_conditions.text_to_be_present_in_element(
                (By.XPATH, '//span[@class="pagination-info"]'), expected
            )
        )
...
class LayertreePage(BasePage):
    ...
    def find_item(self, path, timeout=None):
        return self.find_element(By.CSS_SELECTOR, 'li.jstree-node[id="{}"]'.format(path), timeout)

    def find_item_action(self, path, action, timeout=None):
        dropdown = self.find_element(
            By.CSS_SELECTOR,
            ".jstree-grid-column.actions " '.jstree-grid-cell[data-jstreegrid="' + path + '"] ' ".dropdown",
            timeout=10,
        )
